...
if (!function_exists('crf')) {
    function crf($comprobante_procesar, $uuid_generado)
    {
        ...
        if (isset($comprobante_procesar["detalle"][0]->line_provider_id) && !empty($comprobante_procesar["detalle"][0]->line_provider_id)) {
            // dd($comprobante_procesar[2]);
            $ventaTercero = [
                "nit"       => str_replace("-", "", $comprobante_procesar["detalle"][0]->provider_nit),
                "nombre"    => $comprobante_procesar["detalle"][0]->provider_name,
        ...
        foreach ($cuerpo as $item) {
            ...
            $properties_items_cuerpoDocumento = [
                ...
                "precioUni"         => round((float)$precioUnitario, 8),
                "montoDescu"        => 0.00,
        ...
        $resumen = [
            ...
            "totalExenta"           => round((float)$totales["totalExenta"],2),
            ...
            "porcentajeDescuento"   => round((float)$totales["porcentajeDescuento"],2),
            ...
            //"ivaPerci1"             => round((float)$totales["ivaPerci1"],2),
            "ivaPerci1"             => round((float)"0.00",2),
            ...
            "condicionOperacion"    => round((float)$totales["condicionOperacion"],2),
            "pagos"                 => $pagos,
            "numPagoElectronico"    => ""
        ];
...
if (!function_exists('fac')) {
    function fac($comprobante_procesar, $uuid_generado){
        ...
        $ventaTercero = null;
        //dd($comprobante_procesar["documento"][0]);
        if (isset($comprobante_procesar["detalle"][0]->line_provider_id) && !empty($comprobante_procesar["detalle"][0]->line_provider_id)) {
            // dd($comprobante_procesar[2]);
            $ventaTercero = [
                "nit"       => str_replace("-", "", $comprobante_procesar["detalle"][0]->provider_nit),
        ...
        foreach ($cuerpo as $item) {
            ...
            if($item->gravadas > 0 || $item->iva > 0){
                ...
                $ventagravada = round((float)($item->gravadas + $item->iva), 8);
                $ivaItem = round((float)$item->iva, 8);
            } else {
                $ventagravada = 0;
                $ivaItem = 0;
            }
            ...
            $properties_items_cuerpoDocumento = [
                ...
                "uniMedida"         => intval($item->uniMedida),
                "descripcion"       => $item->descripcion,
                "precioUni"         => round((float)($precioUnitario + $iva_calculadofac), 8),
                "montoDescu"        => 0.00,
                "ventaNoSuj"        => round((float)$item->no_sujetas, 8),
                "ventaExenta"       => round((float)$item->exentas, 8),
                "ventaGravada"      => round((float)$ventagravada, 8),
                "tributos"          =>  null,
                "psv"               => (float)"0.00",
                "noGravado"         => round((float)$item->no_imponible, 8),
                "ivaItem"           => round((float)$ivaItem, 8)
            ];
        ...
        $extension = [
            ...
            "docuRecibe"    => ($es_mayor) ? (
                ...
                (empty(trim($cliente[0]->nit ?? '')) || strlen($cliente[0]->nit ?? '') == 0) ? (
                    // Si es extranjero, usar pasaporte
...
if (!function_exists('clq')) {
    function clq($comprobante_procesar, $uuid_generado)
    {
        ...
        $emisor = [
            "nit"                   => str_replace("-","",$emisor_data[0]->nit),
            "nrc"                   => str_replace("-", "",trim($emisor_data[0]->ncr)),
            "nombre"                => $emisor_data[0]->nombre,
            "codActividad"          => $emisor_data[0]->codActividad,
            "descActividad"         => $emisor_data[0]->descActividad,
            "nombreComercial"       => $emisor_data[0]->nombreComercial,
            "tipoEstablecimiento"   => $emisor_data[0]->tipoEstablecimiento,
        ...
        $tblReceptor = $cliente[0];
        //dd($tblReceptor);
        $direccion_receptor = [
            "departamento"  => $tblReceptor->departamento,
            "municipio"     => $tblReceptor->municipio,
            "complemento"   => $tblReceptor->direccion
        ];
        if (!empty($tblReceptor->codActividad)) {
            if (strlen($tblReceptor->codActividad) <= 4)
            {$codeActivity = '0' . $tblReceptor->codActividad;}
            else{ $codeActivity = $tblReceptor->codActividad; }
        }
        $receptor = [
            "nit"                   => str_replace("-", "",$tblReceptor->nit),
            "nrc"                   => str_replace("-","",trim($tblReceptor->ncr)),
        ...
        if ($tblReceptor->telefono != '') {
            $receptor["telefono"] = $tblReceptor->telefono;
        }
        if ($tblReceptor->correo != '') {
            $receptor["correo"] = $tblReceptor->correo;
        }
        ...
        foreach ($cuerpo as $item) {
            ...
            $properties_items_cuerpoDocumento = [
                "numItem"           => intval($i),
                "tipoDte"           =>$item->clq_tipo_documento,
    ...
    { //dd();
       ...
       // dd($cuerpoDocumento);
        ...
        $resumen = [
            ...
            "ivaPerci1"             => round((float)$encabezado["ivaPerci1"],2),
            "ivaRete1"              => round((float)$encabezado["ivaRete1"]),
