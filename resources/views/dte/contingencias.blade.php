...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="mb-3">
        ...
        @if (session('observaciones_list') && count((array)session('observaciones_list')) > 0)
            <ul class="mt-2 small text-muted">
                @foreach ((array)session('observaciones_list') as $obs)
                    <li>{{ is_string($obs) ? $obs : json_encode($obs) }}</li>
                @endforeach
            </ul>
        @endif
    </div>
