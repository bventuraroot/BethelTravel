...
@section('page-script')
    <script>
        $(document).ready(function() {
            ...
            // If it is a new client, show fields on load
            ...
            // Pre-populate Inclusions
            @if(!empty($quote->includes))
                @foreach($quote->includes as $inc)
                    addInclusion('{{ addslashes($inc) }}');
                @endforeach
            @else
                addInclusion();
            @endif
            ...
            @if(!empty($quote->notes))
                @foreach($quote->notes as $note)
                    addNote('{{ addslashes($note) }}');
                @endforeach
            @else
                addNote();
            @endif

            // Pre-populate Hotels Grid Columns
            @if(!empty($quote->hotels_grid) && !empty($quote->hotels_grid['columns']))
            ...
            @if(!empty($quote->flights))
                @foreach($quote->flights as $f)
                    addFlight({
                        airline_code: '{{ $f['airline_code'] ?? '' }}',
                        flight_number: '{{ $f['flight_number'] ?? '' }}',
                        origin_code: '{{ $f['origin_code'] ?? '' }}',
                        origin_name: '{{ addslashes($f['origin_name'] ?? '') }}',
                        departure_date: '{{ $f['departure_date'] ?? '' }}',
                        departure_time: '{{ $f['departure_time'] ?? '' }}',
                        destination_code: '{{ $f['destination_code'] ?? '' }}',
                        destination_name: '{{ addslashes($f['destination_name'] ?? '') }}',
...
@section('content')
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Cotizaciones /</span> Editar Cotización COT-{{ str_pad($quote->id, 5, '0', STR_PAD_LEFT) }}
    </h4>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <label class="form-label">Reemplazar Imágenes de Banner (Máximo 3)</label>
                                <input type="file" name="banner_images[]" class="form-control" multiple accept="image/*">
                                <small class="text-muted">Deja este campo vacío para mantener las imágenes actuales de la cotización.</small>
                                
                                @if(!empty($quote->banner_images))
                                    <div class="d-flex gap-2 mt-2">
                                        @foreach($quote->banner_images as $img)
                                            <div class="position-relative">
                                                <img src="{{ asset($img) }}" style="height: 60px; width: 90px; object-fit: cover; border-radius: 4px; border: 1px solid #e9ecef;">
                                            </div>
                                        @endforeach
                                    </div>
                                @endif
                    ...
                    <div class="card-body">
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" id="hotels_grid_title" name="hotels_grid_title" class="form-control form-control-sm" placeholder="Ej. HOTELES EL POBLADO" value="{{ $quote->hotels_grid['title'] ?? 'HOTELES Y TARIFAS' }}">
                            </div>
                            <div class="col-md-6">
                                <label for="hotels_grid_footer" class="form-label">Nota de pie de tabla</label>
                                <input type="text" id="hotels_grid_footer" name="hotels_grid_footer" class="form-control form-control-sm" placeholder="Ej. Precios cotizados en base a mínimo..." value="{{ $quote->hotels_grid['footer'] ?? '' }}">
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <select id="client_id" name="client_id" class="select2 form-select" data-allow-clear="true">
                                    ...
                                    @foreach($clients as $c)
                                        ...
                                        @endphp
                                        <option value="{{ $c->id }}" {{ $quote->client_id == $c->id ? 'selected' : '' }}>{{ $name }} ({{ $c->nit }})</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div id="prospect_fields" style="display: none;" class="col-12 border-top pt-3 mt-3">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="client_name" name="client_name" class="form-control" placeholder="Ej. Juan Pérez" value="{{ $quote->client_name }}">
                                </div>
                                <div class="mb-3">
                                    <label for="client_email" class="form-label">Correo Electrónico</label>
                                    <input type="email" id="client_email" name="client_email" class="form-control" placeholder="juan.perez@example.com" value="{{ $quote->client_email }}">
                                </div>
                                <div class="mb-3">
                                    <label for="client_phone" class="form-label">Teléfono</label>
                                    <input type="text" id="client_phone" name="client_phone" class="form-control" placeholder="7777-7777" value="{{ $quote->client_phone }}">
                                </div>
                            </div>
                        </div>
                    </div>
