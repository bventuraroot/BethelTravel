        ...
        <div class="card-header">
            <div class="row align-items-end g-3">
                ...
                <div class="col-md-3 d-flex gap-2">
                    <button type="submit" class="btn btn-danger w-100 fw-bold"><i class="fa-solid fa-magnifying-glass me-1"></i> Buscar</button>
                    @if(isset($sales))
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0 fw-bold"><i class="fa-solid fa-file-circle-minus me-2 text-danger"></i>Notas de Crédito (NCR) - {{ $heading->name ?? '' }} ({{ $period }}/{{ $yearB }})</h5>
        <span class="badge bg-danger fs-6">{{ count($sales) }} registros</span>
    </div>
        ...
        <table class="table table-bordered table-striped table-hover align-middle">
            <thead class="table-dark text-center">
                <tr>
                    ...
                    <th>Doc. Relacionado (Origen)</th>
                    <th>Cliente / Contribuyente</th>
                    <th>Subtotal Ajuste ($)</th>
                    <th>IVA Ajustado ($)</th>
                    <th>Monto Total NCR ($)</th>
                </tr>
            </thead>
            <tbody>
                @php
                ...
                @forelse($sales as $index => $sale)
                    @php
                        ...
                        $iva = (float)($sale->iva ?? 0);
                        $total = (float)($sale->totalamount ?? ($gravada + $iva));
                        $totSubtotal += $gravada;
                        $totIva += $iva;
                        $totTotal += $total;
                    @endphp
                    <tr>
                        ...
                        <td class="text-center fw-bold text-danger">{{ $sale->numeroControl ?? $sale->nu_doc ?? $sale->correlativo }}</td>
                        <td class="text-center"><small class="text-muted">{{ $sale->codigoGeneracion ?? 'N/A' }}</small></td>
                        <td class="text-center fw-semibold text-primary">{{ $sale->doc_relacionado ?? 'N/A' }}</td>
                        <td>{{ $sale->nombre_completo ?? $sale->firstname }}</td>
                        <td class="text-end">$ {{ number_format($gravada, 2) }}</td>
                        <td class="text-end text-danger">$ {{ number_format($iva, 2) }}</td>
                        <td class="text-end fw-bold text-danger">$ {{ number_format($total, 2) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="text-center py-4 text-muted">No se encontraron notas de crédito (NCR) para el período seleccionado.</td>
                    </tr>
                @endforelse
            </tbody>
            @if(count($sales) > 0)
            <tfoot class="table-secondary fw-bold text-end">
                <tr>
                    <td colspan="6" class="text-center">TOTALES GENERALES NCR:</td>
                    <td>$ {{ number_format($totSubtotal, 2) }}</td>
                    <td class="text-danger">$ {{ number_format($totIva, 2) }}</td>
                    <td class="text-danger">$ {{ number_format($totTotal, 2) }}</td>
                </tr>
            </tfoot>
            @endif
        </table>
