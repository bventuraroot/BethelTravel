...
$(function () {
    ...
    $('#btn-export-pdf').on('click', function() {
        ...
        var form = $('<form>', { 'method': 'POST', 'action': '{{ route("report.fse.pdf") }}' });
        form.append($('<input>', { 'type': 'hidden', 'name': '_token', 'value': '{{ csrf_token() }}' }));
        form.append($('<input>', { 'type': 'hidden', 'name': 'company', 'value': company }));
        form.append($('<input>', { 'type': 'hidden', 'name': 'year', 'value': year }));
        form.append($('<input>', { 'type': 'hidden', 'name': 'period', 'value': period }));
    ...
    $('#btn-merge-pdf').on('click', function() {
        ...
        var form = $('<form>', { 'method': 'POST', 'action': '{{ route("report.fse.merge-pdf") }}' });
        form.append($('<input>', { 'type': 'hidden', 'name': '_token', 'value': '{{ csrf_token() }}' }));
        form.append($('<input>', { 'type': 'hidden', 'name': 'company', 'value': company }));
        form.append($('<input>', { 'type': 'hidden', 'name': 'year', 'value': year }));
        form.append($('<input>', { 'type': 'hidden', 'name': 'period', 'value': period }));
        ...
        <table class="table table-sm" style="min-width: 1500px;">
            <thead>
                <tr>
                    <th class="text-center" colspan="15">
                        <b>LIBRO DE COMPRAS A SUJETOS EXCLUIDOS (FSE - DTE 14)</b>
                    </th>
                </tr>
                <tr>
            ...
            <tbody>
                <tr class="text-center">
                    ...
                    <td style="text-align: left; width: 180px;">PROVEEDOR / SUJETO EXCLUIDO</td>
                    <td style="text-align: center; width: 100px;">DUI / NIT</td>
                    <td style="text-align: right; width: 110px;">MONTO BRUTO</td>
                    <td style="text-align: right; width: 110px;">RETENCIÓN RENTA (10%)</td>
                    <td style="text-align: right; width: 110px;">TOTAL LÍQUIDO</td>
                ...
                <tr>
                    ...
                    <td style="text-align: center; padding: 2px 4px;"><?php echo $sale['nit'] ?? $sale['ncrC'] ?? '-'; ?></td>
                    <td style="text-align: right; padding: 2px 4px;">
                        @if($sale['typesale']=='0') 0.00 @else {{ number_format($bruto, 2) }} <?php $tot_bruto += $bruto; ?> @endif
                    </td>
                    <td style="text-align: right; padding: 2px 4px; color: red;">
                        @if($sale['typesale']=='0') 0.00 @else {{ number_format($retencion, 2) }} <?php $tot_retencion += $retencion; ?> @endif
                    </td>
                    <td style="text-align: right; padding: 2px 4px; font-weight: bold; color: green;">
                        @if($sale['typesale']=='0') 0.00 @else {{ number_format($liquido, 2) }} <?php $tot_liquido += $liquido; ?> @endif
                    </td>
                ...
                <tr style="font-weight: bold; background-color: rgba(0,0,0,0.05);">
                    ...
                    <td style="text-align: right;">{{ number_format($tot_bruto, 2) }}</td>
                    <td style="text-align: right; color: red;">{{ number_format($tot_retencion, 2) }}</td>
                    <td style="text-align: right; color: green;">{{ number_format($tot_liquido, 2) }}</td>
                    <td colspan="6"></td>
                </tr>
            </tbody>
        </table>
